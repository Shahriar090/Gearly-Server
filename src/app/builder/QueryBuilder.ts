...
class QueryBuilder<T> {
	...
	constructor(model: Model<T>, query: Record<string, unknown>, useAggregation = false) {
		this.model = model;
		this.query = query;
	...
	filter() {
		...
		const formattedStr = queryStr.replace(/\b(gte|gt|lte|lt|ne|in|nin)\b/g, (match) => `$${match}`);
	...
	paginate() {
		...
		if (this.isAggregation) {
			(this.modelQuery as PipelineStage[]).push({ $skip: skip }, { $limit: limit });
		} else {
	...
	fields() {
		...
		if (fields) {
			if (this.isAggregation) {
				...
				this.modelQuery = (this.modelQuery as Query<T[], T>).select(Object.keys(fields).join(' '));
			}
		}
