...
const getAllCategoriesFromDb = async () => {
	...
	if (!result) {
		throw new AppError(httpStatus.NOT_FOUND, 'No Category Found.!', 'CategoryNotFound');
	}
...
const updateCategoryIntoDb = async (id: string, payload: TCategory) => {
	...
	if (payload.name) {
		...
		if (existingCategory && existingCategory._id.toString() !== id) {
			throw new AppError(httpStatus.CONFLICT, 'Category Name Already Exists.!', 'CategoryNameConflict');
		}
	}
	...
	if (!result) {
		throw new AppError(httpStatus.INTERNAL_SERVER_ERROR, 'Failed To Update Category', 'CategoryUpdateFailed');
	}
