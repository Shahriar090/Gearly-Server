...
import type { TOrder, TOrderStatus, TPaymentStatus } from './order.interface';
...
const createOrderIntoDb = async (payload: TOrder, userId: string) => {
	if (!payload || payload.items.length === 0) {
		throw new AppError(httpStatus.NOT_FOUND, 'No Order Items Found', 'NoOrderItems');
	}
	...
	try {
		...
		if (products.length !== payload.items.length) {
			throw new AppError(httpStatus.NOT_FOUND, 'One Or More Products Were Not Found', 'ProductNotFound');
		}
		...
		for (const item of payload.items) {
			...
			if (!product) {
				throw new AppError(httpStatus.NOT_FOUND, `Product With ID ${item.product} Not Found`, 'ProductNotFound');
			}
...
const getOrderByIdFromDb = async (orderId: string) => {
	...
	if (!order || order.isDeleted) {
		throw new AppError(httpStatus.NOT_FOUND, 'Order Not Found', 'OrderNotFound');
	}
...
const updateOrderStatusIntoDb = async (orderId: string, status: TOrderStatus) => {
	if (!status || !Object.values(ORDER_STATUS).includes(status)) {
		throw new AppError(httpStatus.BAD_REQUEST, 'Invalid Order Status', 'InvalidOrderStatus');
	}
...
const cancelOrderFromDb = async (orderId: string, userId: string) => {
	...
	if (order.user.toString() !== userId) {
		throw new AppError(httpStatus.FORBIDDEN, 'You Are Not Authorized To Cancel This Order', 'UnauthorizedCancelAction');
	}
	if (order && order.orderStatus !== ORDER_STATUS.Pending) {
		// user can only cancel their order if the order status is pending

		throw new AppError(httpStatus.BAD_REQUEST, 'Only Pending Orders Can Be Canceled', 'InvalidCancel');
	}
...
const updatePaymentStatusIntoDb = async (orderId: string, paymentStatus: TPaymentStatus) => {
	if (!paymentStatus || !Object.values(PAYMENT_STATUS).includes(paymentStatus)) {
		throw new AppError(httpStatus.BAD_REQUEST, 'Invalid Payment Status', 'InvalidPaymentStatus');
	}

	const updatedOrder = await Order.findByIdAndUpdate(orderId, { paymentStatus: paymentStatus }, { new: true });
...
const countTotalOrdersFromDb = async () => {
	...
	if (totalOrders === 0) {
		throw new AppError(httpStatus.NOT_FOUND, 'No Orders Found', 'NoOrdersFound');
	}
...
const calculateSalesBetweenDates = async (startDate: string, endDate: string) => {
	const sales = await Order.aggregate([
		{
			$match: {
				isDeleted: false,
