import { z } from 'zod';
import { ORDER_STATUS, PAYMENT_METHODS, PAYMENT_STATUS, DELIVERY_METHODS } from './order.constants';

const createItemValidationSchema = z.object({
	product: z.string().min(1, 'You Have To Add At Least One Product To Place An Order'),
...
const createOrderValidationSchema = z.object({
	body: z.object({
		order: z.object({
			...
			items: z.array(createItemValidationSchema).nonempty('Order Must Have At Least One Item'),
			discount: z.number().min(0).optional(),
			...
			deliveryMethod: z.enum(Object.values(DELIVERY_METHODS) as [string, ...string[]]).optional(),
...
const updateOrderValidationSchema = z.object({
	body: z.object({
		order: z.object({
			user: z.string().optional(),
			trackingId: z.string().optional(),
			items: z.array(updateItemValidationSchema).nonempty('Order Must Have At Least One Item').optional(),
			discount: z.number().min(0).optional(),
			totalSaved: z.number().min(0).optional(),
			tax: z.number().min(0).optional(),
			shippingCharge: z.number().min(0).optional(),
