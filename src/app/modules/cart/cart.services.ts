...
const addToCart = async (userId: string, payload: TCart) => {
	...
	if (products.length === 0) {
		throw new AppError(httpStatus.NOT_FOUND, 'Some Products Not Found', 'ProductsNotFound');
	}
	...
	const newItems = payload.items.map((newItem) => {
		if (newItem.quantity <= 0) {
			throw new AppError(httpStatus.BAD_REQUEST, 'Quantity must be greater than 0', 'InvalidQuantity');
		}

		const product = productMap.get(newItem.product.toString());

		if (!product) {
			throw new AppError(httpStatus.NOT_FOUND, `Product With ID ${newItem.product} Not Found`, 'ProductNotFound');
		}
...
const updateCartItem = async (userId: string, productId: string, quantity: number) => {
	...
	if (itemIndex === -1) {
		throw new AppError(httpStatus.NOT_FOUND, 'Product Not In Cart', 'ProductNotInCart');
	}
	...
	if (quantity > product.stock) {
		throw new AppError(httpStatus.BAD_REQUEST, `Only ${product.stock} Units Available`, 'OutOfStock');
	}
