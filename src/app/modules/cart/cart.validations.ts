...
const createCartValidationSchema = z.object({
	body: z.object({
		cart: z.object({
			...
			shippingCharge: z.number().min(0, 'Shipping charge cannot be negative').default(0),
 ...
 */
const createNewCartValidationSchema = createCartValidationSchema.shape.body.shape.cart.pick({
	user: true,
	items: true,
});
