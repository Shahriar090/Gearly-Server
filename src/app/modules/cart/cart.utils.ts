import type { TCartItems } from './cart.interface';
...
export const mergeCartItems = (existingItems: TCartItems[], newItems: TCartItems[]) => {
	const itemMap = new Map(existingItems.map((item) => [item.product.toString(), item]));
	...
	newItems.forEach((newItem) => {
		if (itemMap.has(newItem.product.toString())) {
			...
			existingItem.saved += newItem.discount != null ? newItem.discount : newItem.price * newItem.quantity;
		} else {
