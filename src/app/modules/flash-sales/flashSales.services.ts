...
// add to flash sales
const createFlashSales = async (payload: { startTime: string; endTime: string; flashSales: TFlashSales[] }) => {
		...
		},
		{
			$addFields: {
				flashSaleDiscountedPrice: {
					$subtract: ['$product.price', { $multiply: ['$product.price', { $divide: ['$discount', 100] }] }],
				},
			},
		},
		{
...
const deleteFlashSale = async (id: string) => {
	...
	if (!product) {
		throw new AppError(httpStatus.NOT_FOUND, 'No Product Found With This Id', 'NotFound');
	}
