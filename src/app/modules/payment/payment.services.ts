...
const paymentSuccess = async (tranId: string) => {
	...
	if (order.paymentStatus === 'Paid') {
		throw new AppError(httpStatus.BAD_REQUEST, `Payment Already Marked As Paid For Order: ${tranId}`);
	}
...
const paymentFailed = async (tranId: string) => {
	...
	if (order.isDeleted) {
		throw new AppError(httpStatus.BAD_REQUEST, 'Order Already Marked As Deleted');
	}
