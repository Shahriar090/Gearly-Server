...
const createSubCategory = asyncHandler(async (req, res) => {
	...
	if (!uploadImage) {
		throw new AppError(httpStatus.BAD_REQUEST, 'Image upload failed.', 'ImageUploadError');
	}
	subCategory.imageUrl = uploadImage;

	const result = await subCategoriesServices.createSubCategoryIntoDb(subCategory);
...
const getAllSubCategoriesWithProductCount = asyncHandler(async (req, res) => {
	const result = await subCategoriesServices.getAllSubCategoriesWithProductCount(req.query);
...
const getSubCategoryByCategory = asyncHandler(async (req, res) => {
	...
	const result = await subCategoriesServices.getSubCategoryByCategory(slug as string);
