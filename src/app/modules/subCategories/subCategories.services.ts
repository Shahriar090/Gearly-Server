...
import type { TSubCategory } from './subCategories.interface';
...
const createSubCategoryIntoDb = async (payload: TSubCategory) => {
	const categoryNameSlug = payload.categoryName.toLowerCase().replace(/\s+/g, '-');
...
const getAllSubCategoriesWithProductCount = async (query: Record<string, unknown>) => {
	...
	const mergeResult = subCategories.map((subCategory: any) => {
		const productCountData = productCounts.find((product) => String(product._id) === String(subCategory._id));
		return {
			...subCategory,
	...
	if (!mergeResult.length) {
		throw new AppError(httpStatus.NOT_FOUND, 'No Sub Category Found.!', 'SubCategoryNotFound');
	}
...
const getSubCategoryByCategory = async (slug: string) => {
	...
	if (result.length === 0) {
		throw new AppError(httpStatus.NOT_FOUND, 'No Sub Category Found With This Category.', 'SubCategoryNotFound');
	}
	return result;
};
...
const updateSubCategory = async (id: string, payload: Partial<TSubCategory>) => {
	...
	const allowedFieldsToUpdate: Array<keyof TSubCategory> = ['brandName', 'description', 'imageUrl'];
	...
	const updatedSubCategory = await SubCategory.findByIdAndUpdate(id, updatedData, { new: true, runValidators: true });
...
const deleteSubcategoryFromDb = async (id: string) => {
	...
	if (subCategory.isDeleted) {
		throw new AppError(httpStatus.BAD_REQUEST, 'Sub-Category Already Deleted', 'SubCategoryAlreadyDeleted');
	}
