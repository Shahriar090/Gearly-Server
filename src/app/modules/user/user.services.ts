...
const getSingleUserFromDb = async (id: string) => {
	...
	if (!result) {
		throw new AppError(httpStatus.NOT_FOUND, 'No User Found With This Id.!', 'UserNotFound');
	}
	return result;
};
...
const updateUserIntoDb = async (id: string, payload: IUser) => {
	...
	if (payload.password) {
		modifiedUpdatedData.password = await bcrypt.hash(payload.password, Number(config.bcrypt_salt_round));
	}
	...
	if (!result) {
		throw new AppError(httpStatus.NOT_FOUND, 'User Not Found', 'UserNotFoundError');
	}
	return result;
};
