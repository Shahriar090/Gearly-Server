...
const createUserValidationSchema = z.object({
	body: z.object({
		user: z.object({
			...
			role: z.enum([USER_ROLES.Admin, USER_ROLES.Customer]).optional().default(USER_ROLES.Customer).optional(),
			status: z.enum([USER_STATUS.Active, USER_STATUS.Blocked]).optional().default(USER_STATUS.Active).optional(),
...
const updateUserValidationSchema = z.object({
	body: z.object({
		user: z.object({
			...
			gender: z.enum([USER_GENDER.Male, USER_GENDER.Female, USER_GENDER.Others]).optional(),
			age: z.number().int().min(18, 'Age Must Be At Least 18').optional(),
			contactNo: z.string().min(1, 'Contact Number Is Required').trim().optional(),
			address: z.string().min(1, 'Address Is Required').trim().optional(),
			email: z.string().email('Invalid Email Format').min(1, 'Email Is Required').trim().optional(),
