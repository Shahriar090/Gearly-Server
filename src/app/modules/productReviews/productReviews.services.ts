...
import type { TProductReview } from './productReviews.interface';
import httpStatus from 'http-status';
import { Review } from './productReviews.model';
import { updateProductRatings } from './productReviews.utils';

// create a review
const createReview = async (userId: string, productId: string, payload: TProductReview) => {
	// check if the product is exists or not
	const product = await Product.findById(productId);

	if (!product || product.isDeleted) {
		throw new AppError(httpStatus.NOT_FOUND, 'No Product Found', 'NoProductFound');
	}
	...
	if (isReviewed) {
		throw new AppError(httpStatus.BAD_REQUEST, 'You Have Already Reviewed This Product', 'AlreadyReviewed');
	}
...
// update a review
const updateReview = async (reviewId: string, userId: string, payload: Partial<TProductReview>) => {
	// find the submitted review
	const submittedReview = await Review.findById(reviewId);

	if (!submittedReview) {
		throw new AppError(httpStatus.NOT_FOUND, 'Review Not Found.!', 'ReviewNotFound');
	}

	// check if the review is belong to the right user or not
	if (!submittedReview.user.equals(userId)) {
		throw new AppError(httpStatus.FORBIDDEN, 'You Are Not Authorized To Update This Review', 'NotAuthorized');
	}
...
const deleteReview = async (reviewId: string, userId: string) => {
	...
	if (!review.user.equals(userId)) {
		throw new AppError(httpStatus.FORBIDDEN, 'You Are Not Authorized To Delete Tis Review', 'NotAuthorized');
	}
